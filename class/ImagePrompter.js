...
var ImagePrompter = function(settings){
  var prompter = {
    options: [
      {
        ...
        events: {
          create: function(el, option){
            setClickFileHandler(el, function(event){
    ...
    loadImageFromFile: function(event, callback){
      return loadFileInput(event, function(data){
    ...
    setup: function(settings){
      ...
      if(!settings.classes.CanvasManager){
        console.error("REQUIRES CanvasManager");
      }
      if(!settings.classes.streamManager){
        console.error("REQUIRES streamManager");
      }
      
      if(settings){
        if(settings.element){prompter.elements.main = settings.element;}
        if(settings.callback){prompter.events.select = settings.callback;}
        if(settings.classes && settings.classes.canvasManager){prompter.classes.canvasManager = settings.classes.canvasManager;}
        if(settings.classes && settings.classes.streamManager){prompter.classes.streamManager = settings.classes.streamManager;}
      }
    ...
    show: function(){
      ...
      */
      ...
      swal({//https://limonte.github.io/sweetalert2/
        ...
        onOpen: function(el){$(el).prepend(prompter.elements.list);},
